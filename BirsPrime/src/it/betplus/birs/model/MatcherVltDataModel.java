...
public class MatcherVltDataModel extends
		ListDataModel<Matcher> implements java.io.Serializable {

	private static final long serialVersionUID = 1L;

	public MatcherVltDataModel() {
	}

	public MatcherVltDataModel(List<Matcher> data) {
		super(data);
	}
	...
	public Matcher getRowData(String rowKey) {
		...
		for (Matcher m : modelData) {
			if ((m.getTempodim().simpleTempodimToString()+"-"+m.getSistemagiocodim().getVlt().getAAMS_VLT_ID()).equals(rowKey))
				return m;
		}
	...
	public Object getRowKey(Matcher m) {
		return m.getTempodim().simpleTempodimToString()+"-"+m.getSistemagiocodim().getVlt().getAAMS_VLT_ID();
	}
