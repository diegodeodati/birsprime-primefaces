...
@ManagedBean(name = "tableBeanLocation")
@SessionScoped
public class TableBeanLocation extends BaseTableBean {
	...
	private List<MeterfactLocation> filteredTableList;
	...
	public void setListDataFromSource(ArrayList<Object> params)
		...
		try {
			...
			List<MeterfactLocation> m = MeterDAO.getInstance()
					.retrieveMeter_locations(reportBean.getDataS(),
							reportBean.getDataE(),
		...
		if (selectedObj != null) {
			try {
				if (!actualSelected.equals(selectedObj.getSistemagiocodim()
					...
					actualSelected = selectedObj.getSistemagiocodim().getLoc()
							.getAAMS_LOCATION_ID();
