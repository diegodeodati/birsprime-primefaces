...
@ManagedBean(name = "tableBeanVlt")
@SessionScoped
public class TableBeanVlt extends BaseTableBean {
	...
	public TableBeanVlt() throws DataLayerException {
	...
	public void setListDataFromSource(ArrayList<Object> params)
		...
		try {
			...
			List<MeterfactVlt> m = MeterDAO.getInstance().retrieveMeter_vlts(
	...
	public void setFilteredTableList(List<MeterfactVlt> filteredTableList) {
		TableBeanVlt.filteredTableList = filteredTableList;
	}
	...
	public void refreshTableList() {
		...
		actTableList = (ArrayList<MeterfactVlt>) this.getDataModel().getData();

		actTableList = (ArrayList<MeterfactVlt>) filterOnDay(actTableList,
				rBean);
		actTableList = (ArrayList<MeterfactVlt>) filterOnClose(actTableList,
				rBean);
	...
	public List<MeterfactVlt> filterOnDay(ArrayList<MeterfactVlt> list,
			ReportBean rBean) {
		...
		if (rBean.isModSingTotDay()) {
			...
			for (MeterfactVlt mSum : list) {
				if (hSum.containsKey(mSum.getSistemagiocodim().getLoc()
					...
					MeterfactVlt mHash = hSum.get(mSum.getSistemagiocodim()
	...
	public List<MeterfactVlt> filterOnClose(ArrayList<MeterfactVlt> list,
			ReportBean rBean) {
	...
	public List<MeterfactVlt> filterOnBet(ArrayList<MeterfactVlt> list,
			ReportBean rBean) {
	 ...
	 * public void refreshTableList() {
	 * 
	 * ReportBean reportBean = findBean("reportBean"); ArrayList<MeterfactVlt>
	 * tempTableList = new ArrayList<MeterfactVlt>();
	 * 
	 * for (MeterfactVlt m : this.getDataModel().getData()) { switch
