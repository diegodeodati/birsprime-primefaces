...
@ManagedBean(name = "tableBeanGame")
@SessionScoped
public class TableBeanGame extends BaseTableBean {
	...
	public TableBeanGame() throws DataLayerException {
	...
	public void setListDataFromSource(ArrayList<Object> params)
		...
		try {
			...
			List<MeterfactGame> m = MeterDAO.getInstance().retrieveMeter_games(
	...
	public void setFilteredTableList(List<MeterfactGame> filteredTableList) {
		TableBeanGame.filteredTableList = filteredTableList;
	}
		...
		if (rBean.isModSingTotDay()) {
			...
			for (MeterfactGame mSum : list) {
				if (hSum.containsKey(mSum.getSistemagiocodim().getLoc()
						...
						+ "-"
						+ mSum.getSistemagiocodim().getGame().getAAMS_GAME_ID())) {
					...
					MeterfactGame mHash = hSum.get(mSum.getSistemagiocodim()
							...
							+ mSum.getSistemagiocodim().getVlt()
									.getAAMS_VLT_ID()
							+ "-"
							+ mSum.getSistemagiocodim().getGame()
									.getAAMS_GAME_ID());
